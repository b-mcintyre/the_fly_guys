...
m3 <- update(m2, . ~ . - (0 + Allele_1 | WT_Background) + (1|WT_Background/Allele_1))
VarCorr(m3)
...
m5 <- glmmTMB(lev_stat ~ Allele_1 + rr(0 + Allele_1 | WT_Background,5) + Replicate,
              data=wd, control=glmmTMBControl(optCtrl=list(iter.max=1000,eval.max=1000)))
v5 <- cov2cor(VarCorr(m5)$cond[[1]])
plot_grid(ifun(v4,main="full"),ifun(v5, main="rr5"))
